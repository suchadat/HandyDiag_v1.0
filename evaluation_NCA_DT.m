...
remarks = sprintf("Remark: Cleaned Data, Z-Score Norm, NCA, Decision Tree with BayesOpt(%d evals)", max_obj_evals);
time_str = char(datetime('now', 'Format', 'dd_MMMM_yyyy_''time''_HH_mm_ss', 'Locale', 'en_US'));
filename = sprintf('Tree_NCA_20topfeature_%s.txt', time_str);   
...
for task = 1:6     
    ...
    switch task
        ...
        case 5
            ...
            task_name = 'Vertical';
        case 6
            % --- FIX 2: Safe Data Alignment using innerjoin ---
    ...
    % --- FIX 3: Prevent Stratification Mismatch ---
    ...
    parfor r = 1:numShuffles
        ...
        for k = 1:numFolds
            ...
            Tree_model = fitctree(XTrain_Final, yTrain, ...
                     'OptimizeHyperparameters', 'auto', ...
                     'HyperparameterOptimizationOptions', current_hyperopts);
                
            pd_idx = find(string(Tree_model.ClassNames) == pd_category);
            if isempty(pd_idx); pd_idx = 1; end
            
            % --- E. Evaluate ---
            [ytrainPred, scores_train] = predict(Tree_model, XTrain_Final);
            fold_train_metrics(k, :) = get_metrics(yTrain, ytrainPred, scores_train, pd_idx);
            
            [yvalPred, scores_test] = predict(Tree_model, XTest_Final);
    ...
    writetable(Final_Stats_Train, sprintf('Tree_NCA_Train_%s.csv', task_name));
    
    Final_Stats_Test = array2table([(1:numShuffles)', Result_Test], 'VariableNames', varNames);
    writetable(Final_Stats_Test, sprintf('Tree_NCA_Test_%s.csv', task_name));
