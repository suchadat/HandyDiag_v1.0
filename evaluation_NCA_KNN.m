...
% NCA feature counts for Tasks 1-5 vs Task 6
...
remarks = sprintf("Remark: Cleaned Data, Z-Score Norm, NCA, KNN with BayesOpt(%d evals)", max_obj_evals);
time_str = char(datetime('now', 'Format', 'dd_MMMM_yyyy_''time''_HH_mm_ss', 'Locale', 'en_US'));
filename = sprintf('KNN_NCA_20topfeature_%s.txt', time_str);   
...
for task = 1:6     
    ...
    switch task
        ...
        case 5
            ...
            task_name = 'Vertical';
        case 6
            % --- FIX 3: Safe Data Alignment using innerjoin ---
            % Load Controls
    ...
    % --- FIX 2: Prevent Stratification Mismatch ---
    ...
    parfor r = 1:numShuffles % Changed from parfor to for due to dynamic struct assignment limitations in parallel pools
        ...
        for k = 1:numFolds
            ...
            % --- FIX 1: Secondary Leakage (Subject-wise Inner CV for BayesOpt) ---
            ...
            try
                % Attempt to overwrite indices (Works in many MATLAB environments)
                inner_cv_final.Impl.indices = inner_row_partition;
                current_hyperopts.CVPartition = inner_cv_final;
            catch
                % Silently falls back to default auto inner-CV if the environment restricts object mutation
            end
            
            % --- D. Train KNN ---
            KNN_model = fitcknn(XTrain_Final, yTrain, ...
                     'Standardize', false, ...
                     'Prior', 'empirical', ... 
                     'OptimizeHyperparameters', 'auto', ...
                     'HyperparameterOptimizationOptions', current_hyperopts);
                
            pd_idx = find(string(KNN_model.ClassNames) == pd_category);
            if isempty(pd_idx); pd_idx = 1; end
            
            % --- E. Evaluate ---
            [ytrainPred, scores_train] = predict(KNN_model, XTrain_Final);
            fold_train_metrics(k, :) = get_metrics(yTrain, ytrainPred, scores_train, pd_idx);
            
            [yvalPred, scores_test] = predict(KNN_model, XTest_Final);
    ...
    %% 2.6 Save Results and Extract Top 20 Features
    ...
    writetable(Final_Stats_Train, sprintf('KNN_NCA_Train_%s.csv', task_name));
    
    Final_Stats_Test = array2table([(1:numShuffles)', Result_Test], 'VariableNames', varNames);
    writetable(Final_Stats_Test, sprintf('KNN_NCA_Test_%s.csv', task_name));
