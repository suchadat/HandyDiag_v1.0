...
%% --------------- Writing letter and sentence task ---------------------------
function [Features_Wl, Features_THl] = extract_letters_feature(currentSubfolderPath, decode, fs_target, fc)

% --- Metadata Extraction ---
DominantHand = string(decode.DominantHand);
Subject_ID = string(decode.ID);

%% 1. Process Words (Wl) Data
[Wl_signals, Wl_summary] = process_core_logic(decode, 'Letters', DominantHand, fs_target, fc);
Features_Wl = compile_feature_table(Wl_signals, Wl_summary, Subject_ID, 'Wl');

% Save Wl Result
save(fullfile(currentSubfolderPath, Subject_ID + "_MWl.mat"), 'Features_Wl');

%% 2. Process Sentences (THl) Data
[THl_signals, THl_summary] = process_core_logic(decode, 'THLetters', DominantHand, fs_target, fc);
Features_THl = compile_feature_table(THl_signals, THl_summary, Subject_ID, 'THl');

% Save THl Result
save(fullfile(currentSubfolderPath, Subject_ID + "_MTHl.mat"), 'Features_THl');

end

%% --- Helper: Core Logic (Cleaning, Stroke Detection, Resampling, Tremor) ---
function [signals, kin_summary] = process_core_logic(decode, prefix, DominantHand, fs_target, fc)
    % Define field names based on task prefix
    ...
    % % Get raw kinematic summary before resampling (Optional/Traditional)
    ...
    X_tremor_all = []; Y_tremor_all = []; P_tremor_all = [];
    ...
    for i = 1:(length(stroke_start_idx)-1)
        ...
        X_tremor_all = [X_tremor_all; X_tremor];
        Y_tremor_all = [Y_tremor_all; Y_tremor];
        P_tremor_all = [P_tremor_all; P_tremor];
    end
    
    X_tremor_all = filtfilt(b, a_filt, X_tremor_all);
    Y_tremor_all = filtfilt(b, a_filt, Y_tremor_all);
    P_tremor_all = filtfilt(b, a_filt, P_tremor_all);
    ...
    % --- 6. Kinematic Feature Extraction ---
    ...
    %Package signals for compilation
    signals.P = P_res_all; signals.X = X_res_all; signals.Y = Y_res_all;
    signals.   = P_tremor_all; signals.Xtrem = X_tremor_all; signals.Ytrem = Y_tremor_all;
...
%% --- Helper: Compile Stats into Table ---
function Tbl = compile_feature_table(S, K, ID, tag)
    ...
    Tbl = array2table([0, current_data], 'VariableNames', all_names); % Placeholder 0 for ID in array
    Tbl.Subject_ID = ID;
end